...
fn array_to_utf8_json_array(py: Python, array: &Bound<'_, PyAny>, large: bool) -> PyResult<PyObject> {
    ...
    let schema = arrow::datatypes::Schema::new(vec![
        arrow::datatypes::Field::new("json", array.data_type().clone(), true)
    ]);
    ...
    if large {
        ...
        for value in json_values.iter() {
            match value {
                ...
                value => builder.append_value(to_string(&value)
                    .map_err(|e| PyValueError::new_err(format!("Failed to serialize JSON value: {:?}", e)))?),
            }
        }
