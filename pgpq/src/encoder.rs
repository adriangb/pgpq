...
pub(crate) enum Encoder<'a> {
    ...
    Time64Nanosecond(Time64NanosecondEncoder<'a>),
    Time64Microsecond(Time64MicrosecondEncoder<'a>),
    DurationNanosecond(DurationNanosecondEncoder<'a>),
...
impl_encode!(Date64Encoder, 8, identity, BufMut::put_i64);
...
#[derive(Debug, Clone)]
pub(crate) struct Time64NanosecondEncoder<'a> {
    arr: &'a arrow_array::Time64NanosecondArray,
}
impl_encode!(
    Time64NanosecondEncoder,
    8,
    |v| v / 1_000_000,
    BufMut::put_i64
);
...
#[derive(Debug, Clone)]
pub(crate) struct DurationNanosecondEncoder<'a> {
    arr: &'a arrow_array::DurationNanosecondArray,
}
impl_encode!(
    DurationNanosecondEncoder,
    8,
    |v| v / 1_000_000,
    BufMut::put_i64
);
...
impl_encode!(DurationMicrosecondEncoder, 8, identity, BufMut::put_i64);
...
impl<'a, T: OffsetSizeTrait> Encode for GenericStringEncoder<'a, T> {
    fn encode(&self, row: usize, buf: &mut BytesMut) -> Result<(), Error> {
        if self.arr.is_null(row) {
            ...
            // TODO: get access to underlying data without going bytes -> str -> bytes
...
impl_encoder_builder_stateless!(Date64EncoderBuilder, Encoder::Date64, Date64Encoder);
...
#[derive(Debug, Clone)]
pub(crate) struct Time64NanosecondEncoderBuilder {
    field: String,
}
impl_encoder_builder_stateless!(
    Time64NanosecondEncoderBuilder,
    Encoder::Time64Nanosecond,
    Time64NanosecondEncoder
);
...
#[derive(Debug, Clone)]
pub(crate) struct DurationNanosecondEncoderBuilder {
    field: String,
}
impl_encoder_builder_stateless!(
    DurationNanosecondEncoderBuilder,
    Encoder::DurationNanosecond,
    DurationNanosecondEncoder
);
...
pub(crate) enum EncoderBuilder {
    ...
    Time64Nanosecond(Time64NanosecondEncoderBuilder),
    Time64Microsecond(Time64MicrosecondEncoderBuilder),
    DurationNanosecond(DurationNanosecondEncoderBuilder),
    ...
    pub(crate) fn try_new(data_type: &DataType, field: &str) -> Result<Self, Error> {
        ...
        let res = match data_type {
            ...
            DataType::Time64(unit) => match unit {
                TimeUnit::Nanosecond => {
                    Self::Time64Nanosecond(Time64NanosecondEncoderBuilder { field })
                }
                TimeUnit::Microsecond => {
            ...
            DataType::Duration(unit) => match unit {
                TimeUnit::Nanosecond => {
                    Self::DurationNanosecond(DurationNanosecondEncoderBuilder { field })
                }
                TimeUnit::Microsecond => {
            ...
            _ => return Err(Error::type_unsupported(&field, data_type)),
        };
        Ok(res)
    }
}
