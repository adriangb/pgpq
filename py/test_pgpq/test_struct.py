...
def parse_composite_value(value: str, field: pa.Field) -> Any:
    """Parse Postgres composite type (struct) or array string representation to Python tuple/list."""
    if value is None:
        return None
    ...
    if pa.types.is_list(field.type) or pa.types.is_large_list(field.type):
        ...
        return [parse_composite_value(e, element_field) if e else None for e in elements]
...
def test_basic_struct(dbconn: Connection) -> None:
    ...
    parsed_rows = [
        (parse_composite_value(row[0], struct_field),)
        for row in rows
    ]
...
def test_array_of_structs(dbconn: Connection) -> None:
    ...
    try:
        ...
        parsed_rows = [
            (parse_composite_value(row[0], list_field),)
            for row in rows
        ]
        
        # First row should have list with THREE structs: [(1, "a"), (2, "b"), (3, "c")]
        # Second row should have list with ONE struct: [(4, "d")]
        assert parsed_rows[0] == ([(1, "a"), (2, "b"), (3, "c")],), f"Expected ([(1, 'a'), (2, 'b'), (3, 'c')],), got {parsed_rows[0]}"
        assert parsed_rows[1] == ([(4, "d")],), f"Expected ([(4, 'd')],), got {parsed_rows[1]}"
        
        print(f"✓ Arrays of structs work! Row 1: {parsed_rows[0]}, Row 2: {parsed_rows[1]}")
